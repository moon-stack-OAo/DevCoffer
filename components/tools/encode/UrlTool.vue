...
import { decodeUrl, encodeUrl } from '#shared/encode/url'
...
function doEncode() {
    clearError()
    try {
        setOutput(encodeUrl(input.value))
    } catch (e) {
...
function doDecode() {
    clearError()
    try {
        setOutput(decodeUrl(input.value))
    } catch (e) {
...
<template>
  <UiToolShell title="URL 编解码" :error="error">
    <template #actions>
    ...
    <template #input>
      ...
      <textarea v-model="input" class="ta" rows="12" placeholder="文本或 URL 编码串…" />
    </template>
    <template #output>
