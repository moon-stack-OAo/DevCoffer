...
export const QC_MONTH_NAMES: Record<string, number> = {
  JAN: 1,
  FEB: 2,
  MAR: 3,
  APR: 4,
...
export const QC_DOW_NAMES: Record<string, number> = {
  SUN: 1,
  MON: 2,
  TUE: 3,
  WED: 4,
...
export const QC_DOW_LABELS: Record<number, string> = {
  1: '周日',
  2: '周一',
  3: '周二',
  4: '周三',
...
export const QC_MONTH_LABELS: Record<number, string> = {
  1: '1月',
  2: '2月',
  3: '3月',
  4: '4月',
...
export const QC_PRESETS = [
  { label: '每5分钟', expr: '0 0/5 * * * ?' },
  { label: '每小时', expr: '0 0 * * * ?' },
  { label: '每天零点', expr: '0 0 0 * * ?' },
  { label: '每天中午', expr: '0 0 12 * * ?' },
  { label: '工作日 9:00', expr: '0 0 9 ? * MON-FRI' },
  { label: '每月1号', expr: '0 0 0 1 * ?' },
  { label: '每月最后一天', expr: '0 0 0 L * ?' },
] as const

export interface QuartzCronParsed {
  raw: string
  fields: string[]
  second: string
  minute: string
...
export type SpringScheduledMode = 'cron' | 'fixedRate' | 'fixedDelay'

export interface SpringScheduledOptions {
  cron?: string
  zone?: string
  fixedRate?: number | string
  fixedDelay?: number | string
  initialDelay?: number | string
  methodName?: string
  mode?: SpringScheduledMode
}

export interface SpringScheduledResult {
  cron: string
  fixedRate: string
  fixedDelay: string
  all: string
...
function qcReplaceNames(field: string, nameMap: Record<string, number>): string {
  return String(field).replace(/[A-Za-z]+/g, (w) => {
    const up = w.toUpperCase()
    return nameMap[up] != null ? String(nameMap[up]) : w
  })
}

export function parseQuartzCron(expr: string): QuartzCronParsed {
  const result: QuartzCronParsed = {
  ...
  if (parts.length !== 6 && parts.length !== 7) {
    result.errors.push(
      `Quartz cron 须为 6 段（秒 分 时 日 月 周）或 7 段（含年），当前 ${parts.length} 段`,
    )
    if (parts.length === 5) {
  ...
  function checkField(name: string, val: string | null, min: number, max: number, allowSpecial = true) {
    if (val == null || val === '') {
...
function qcDescribeField(
  label: string,
  val: string,
  opts: { dow?: boolean; month?: boolean } = {},
): string {
  ...
  if (step) {
    ...
    if (base.includes('-')) return `${base} 范围内每${s}${label}`
...
/** 人类可读描述 */
export function describeQuartzCron(exprOrParsed: string | QuartzCronParsed): string {
...
export function toSpringScheduled(options: SpringScheduledOptions = {}): SpringScheduledResult {
  ...
  function delayPart() {
    ...
    if (!isFinite(n)) return `, initialDelay = ${JSON.stringify(String(initialDelay))}`
  ...
  if (cron) {
    cronSnippet =
      `@Scheduled(cron = "${cron.replace(/\\/g, '\\\\').replace(/"/g, '\\"')}"` +
      zonePart() +
      ')\n' +
      methodBody()
  }
  ...
  if (fixedRate != null && fixedRate !== '') {
    ...
    rateSnippet = `@Scheduled(fixedRate = ${rateArg}${delayPart()})\n${methodBody()}`
  }
  ...
  if (fixedDelay != null && fixedDelay !== '') {
    ...
    delaySnippet = `@Scheduled(fixedDelay = ${delayArg}${delayPart()})\n${methodBody()}`
  }
...
export function quartzVsUnixNotes(): string {
  return [
...
/** 格式化解析面板输出 */
export function formatQuartzParseResult(expr: string): {
  desc: string
  text: string
  valid: boolean
  parsed: QuartzCronParsed
} {
  ...
  lines.push(
    `段数: ${parsed.length}${parsed.length === 7 ? '（含年）' : parsed.length === 6 ? '（标准）' : ''}`,
  )
