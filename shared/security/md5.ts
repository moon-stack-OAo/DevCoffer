/** MD5 纯 JS（RFC 1321），供 hash / hmac 使用 */
...
export function hmacMd5Bytes(key: Uint8Array, data: Uint8Array): Uint8Array {
    let k = key
    if (k.length > MD5_BLOCK) k = md5Bytes(k)
    ...
    const inner = new Uint8Array(MD5_BLOCK + data.length)
    inner.set(iKey)
    inner.set(data, MD5_BLOCK)
    const innerHash = md5Bytes(inner)
    const outer = new Uint8Array(MD5_BLOCK + innerHash.length)
    outer.set(oKey)
    outer.set(innerHash, MD5_BLOCK)
